    ...
    return (
        <div className='flex flex-col justify-center w-full items-center mt-2'>
            <form onSubmit={handleSubmit} className='flex flex-col p-5 w-1/4'>
                ...
                <input type="password" minLength={8} pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" placeholder="New password" name="password" onChange={handleChange} className='bg-gray-200 ease-in-out duration-200 p-2 m-2 hover:border-[#098C4C] active:border-[#098C4C] border-2 rounded-lg' />
